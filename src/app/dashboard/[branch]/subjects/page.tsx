...
export default function SubjectsPage() {
  ...
  // For physics and chemistry, use their cycle names
  let semesterNumber: number | string
  if (branchCode === 'PHYSICS') {
    semesterNumber = 'Physics Cycle'
  } else if (branchCode === 'CHEMISTRY') {
    semesterNumber = 'Chemistry Cycle'
  } else {
    // For other branches, this page shouldn't be used directly
    // They should have semester selection first
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 flex items-center justify-center">
        <div className="text-center space-y-6">
          ...
          <p className="text-gray-600">Please select a semester first for this branch.</p>
          <Button onClick={() => router.push(`/dashboard/${branchSlug}`)}>
            <ArrowLeft className="w-4 h-4 mr-2" />
            Back to {branchData?.name || 'Branch'} Selection
          </Button>
        </div>
      </div>
    )
  ...
  // Get subjects - try both branch and cycle
  let subjects: any[] = []
  if (branchCode) {
    subjects = getSubjectsBySemester(branchCode, semesterNumber)
    
    // Debug logging in development
    if (process.env.NODE_ENV === 'development') {
      console.log('Debug - Branch Code:', branchCode)
      console.log('Debug - Semester:', semesterNumber)
      console.log('Debug - Branch Data:', branchData)
      console.log('Debug - Subjects found:', subjects.length)
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredSubjects.map((subject) => (
              <SubjectCard 
                key={subject.code} 
                subject={subject}
                onSelect={() => router.push(`/dashboard/${branchSlug}/subjects/${subjectToSlug(subject.name)}`)}
        ...
        {filteredSubjects.length === 0 && subjects.length === 0 && (
          <div className="text-center py-16 bg-gray-50 rounded-3xl">
            ...
            <p className="text-gray-500 mb-8 max-w-md mx-auto text-lg">
              Subjects for this branch will be added soon. Check back later or contact support.
            </p>
